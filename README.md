...
**首屏内联一张音高显示的 SVG**（`src/components/Tuner.tsx`）。

改版之前这个站从头到尾**一张图都没有** —— 一个卖「唱的当下就看见自己唱准了」
的工具，官网看不到那个显示，用户得读三屏文字才能想象它长什么样。
这是最大的一处缺失，不是配色问题。

用 SVG 不用截图：截图要跟着界面改、要出多份 DPI，而内联 SVG 直接进 HTML，
不增加一次请求、不增加一个字节的 JS（零 JS 这条不能破）。

**延迟构成从表格改成堆叠条。** 五行数字远不如一根条 ——
要让人看见的是"它贴着 30 ms 那条线"（实测占满量程的 99.7%），
而不是让人自己把 2.00 + 9.50 + 3.00 + 15.42 加起来。
设备 I/O 与 DSP 用两种颜色，因为那是两类东西：一个是硬件给的，
一个是算法要的 —— 混成一色就看不出哪一段还有得优化。
...
- 托盘图标颜色是"声卡现在是不是被占着"唯一持续可见的信号
- 标题栏关闭按钮的 tooltip 提前写明「收进托盘（退出请用托盘菜单）」——
  关闭不退出是反直觉的，不能等用户点完发现窗口没了才知道

**隐藏与退出全部在 Rust 侧完成，不绕前端。** 前端只要有一处没响应，
窗口就关不掉了；而托盘的「退出 wego-voice」同样是纯 Rust 的 ——
整条关闭/退出链路不依赖 WebView 是否健康。

退出时**显式停引擎**，不靠 `Drop` 时序（退出路径上托管状态的析构没有保证，

 ...
 * 一根柱子的一档实现。
 *
 * 同一件事分「实时」与「离线」两档，是因为它们的物理约束正好相反：
 * 实时必须压进 30 毫秒（所以只能是纯 DSP、不能回看、不能用神经网络），
 * 离线没有这条约束（所以能回看邻域、能跑模型）。
 *
 * 谁都改不了对方 —— 这不是缺陷，是产品结构。
 */
 ...
 * 两根柱子。
 *
 * ⚠️ 这里写的必须是**下载下来的那个 exe 现在就能做的事**，
 * 不是仓库主干上的进度。开发中的东西只能标「开发中」，
 * 不能用现在时描述。
 */
    ...
    tiers: [
      {
        ...
        points: [
          "端到端实测 **29.92 毫秒**，压在 30 毫秒硬线之下",
          "颤音会保留，不会把人唱成 MIDI",
          "按调式量化，不是简单吸附到最近半音；跑调太离谱时不硬拉",
        ],
        status: "done",
      },
      {
        when: "录完之后",
        title: "离线重新校准",
        body: "实时那条路被 30 毫秒捆着：不能回看、基频只能用因果算法、合成窗口被下限截断。离线重跑这三条限制一条都没有。",
        points: [
          "**八度纠错** —— 拿邻域中位数揪出翻倍/减半的离群帧，实时看不到邻域",
          "**零相位平滑** —— 前向一遍反向一遍，修正不再慢半拍",
          "产物与干声等长，在录音页里并排 A/B",
        ],
        status: "done",
      },
  ...
  },
  {
    ...
    tiers: [
      ...
      },
      {
        when: "录完之后",
        title: "神经声线转换",
        body: "「像某个指定的人」那一档。它做不进 30 毫秒（内容编码要看上下文），只能放在录制之后。目前仍在开发，**没有包含在可下载的版本里**。",
        points: [
          "设计上全程在你自己的电脑上跑，**声音不上传**",
          "主程序体积不会变 —— 模型只在要用这个功能时才下载",
          "⚠️ **现在下载的版本里没有这个功能**",
        ],
        status: "wip",
      },
